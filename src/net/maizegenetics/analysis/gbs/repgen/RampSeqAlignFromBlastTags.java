    ...
    public DataSet processData(DataSet input) {
        ...
        try {           
           ...
           //Multimap<Tag,Position> refTagPositionMap = Multimaps.synchronizedMultimap(HashMultimap.<Tag,Position>create());
            ...
            Multimap<Tag,Tuple<Double,Position>> refTagPositionMap = HashMultimap.create();
            boolean success = createRefTagsFromBlast(blastFile(), refTagPositionMap);
//            if (!success) {
...
//            calculateRefRefAlignment(refTagList,refTagPositionMap,refTagAlignInfoMap, repGenData);
    ...
    public boolean createRefTagsFromBlast(String chromStartEndFile, Multimap<Tag,Tuple<Double,Position>> refTagPositionMap) {
        ...
        //sequence, position, maximum identity
        Set<Position> uniquePhysicalPosition;
        ...
        try {
            ...
            uniquePhysicalPosition= Files.lines(Paths.get(chromStartEndFile))
                    .map(line -> line.split("\t"))
...
//            }
            System.out.println("Read " + uniquePhysicalPosition.size() + " lines from BLAST input file.");
        } catch (Exception exc) {
            exc.printStackTrace();
            return false;
        }
